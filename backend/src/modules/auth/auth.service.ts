...
export default class AuthService {
  ...
  async signup(input: SignupDto): Promise<Session> {
    const { username, email, password, client } = input;
  ...
  async loginByEmail(input: LoginByEmailDto): Promise<Session> {
  ...
  async loginByUsername(input: LoginByUsernameDto): Promise<Session> {
    const { username, password, client } = input;
    const user = await this.userService.getByUserByUsername(username);
    if (
      !user ||
      !(await this.userService.verifyPassword(password, user.passwordHash)) ||
      user.deletedAt
  ...
  async logout(input: LogoutDto): Promise<void> {
    // It will return 204 status code.
